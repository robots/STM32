...
frac vector_dot(struct vector_t* v1, struct vector_t *v2) {
	return fp_mul(v1->coef[0], v2->coef[0]) + 
		fp_mul(v1->coef[1], v2->coef[1]) +
		fp_mul(v1->coef[2], v2->coef[2]);  
}

struct vector_t* vector_cross(struct vector_t *out, struct vector_t* v1, struct vector_t *v2) {
	out->coef[0] = fp_mul(v1->coef[1], v2->coef[2]) - fp_mul(v1->coef[2], v2->coef[1]);
	out->coef[1] = fp_mul(v1->coef[2], v2->coef[0]) - fp_mul(v1->coef[0], v2->coef[2]);
	out->coef[2] = fp_mul(v1->coef[0], v2->coef[1]) - fp_mul(v1->coef[1], v2->coef[0]);
	return out;
}
...
void vector_scale(struct vector_t *v, frac len) {
	v->coef[0] = fp_mul(len, v->coef[0]);
	v->coef[1] = fp_mul(len, v->coef[1]);
	v->coef[2] = fp_mul(len, v->coef[2]);
}

void vector_normalize(struct vector_t *v) {
	frac len = vector_len(v);
	v->coef[0] = fp_div(v->coef[0], len);
	v->coef[1] = fp_div(v->coef[1], len);
	v->coef[2] = fp_div(v->coef[2], len);
}
