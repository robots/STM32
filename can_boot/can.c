...
uint8_t can_send(struct can_message_t *m)
{
	...
	CAN1->sTxMailBox[mailbox].TIR |= m->flags & CAN_MSG_RTR ? 0x02 : 0;

	// add msg ID (cob_id)
	CAN1->sTxMailBox[mailbox].TIR |= m->id << 21;
...
/* TODO: make this interrupt routine again and make it call canfestival callback (or something) */
...
uint8_t can_receive(struct can_message_t *m)
{
	...
	m->flags = CAN1->sFIFOMailBox[fifo].RIR & 0x02 ? CAN_MSG_RTR : 0;
	m->flags |= CAN1->sFIFOMailBox[fifo].RDTR & CAN_MSG_SIZE;
